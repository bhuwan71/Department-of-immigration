...
import { LanguageEnum, multiLanguage } from '@type/global.types' // Import a custom user type.
...
interface IProps {
  ...
  title: multiLanguage // The title or label for the menu item.
...
const SidebarItem = (props: IProps) => {
  ...
  const adminRoleLevel = adminDetails?.admin?.role
      ...
      >
        <Box>
          ...
          >
            ...
            >
              {icon ?? <Icon />}
              {!minimize && title?.[lang]?.charAt(0)?.toUpperCase() + title?.[lang].slice(1)?.toLowerCase()}
            </Flex>
                ...
                ((adminRoleLevel && item.roleLevel?.includes(adminRoleLevel)) ?? item.roleLevel === undefined) && (
                  ...
                  >
                    ...
                    >
                      {item?.icon ?? <Icon />}
                      {!minimize &&
                        item?.title?.[lang]?.charAt(0)?.toUpperCase() + item?.title?.[lang]?.slice(1)?.toLowerCase()}
                    </ListItem>
                  </Link>
                )
              )
      ...
      {isSubMenuVisible && (
        ...
        >
          <AccordionItem border={'0'} my='1'>
            {renderLink(title, minimize, link, icon, shouldRenderSubMenu, lang)}
